...
export default function SerberAlatKerjaTable() {
  const role = useAuthStore((s) => s.role);
  const { data, loading, fetchData } = useFetch<SerberAlatKerjaData>(
  ...
  const [selected, setSelected] = useState<SerberAlatKerjaData | null>(null);

  const handleEdit = async (no_registrasi: string) => {
    try {
      const res = await api.get(`/api/servisberkalaalatkerja/${no_registrasi}`);
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {(search ? filtered : paginatedData).map((item) => {
                ...
                return (
                  <TableRow key={item.id_serberalatkerja}>
                    {columns.map((col, idx) => (
                      <TableCell
                        key={idx}
