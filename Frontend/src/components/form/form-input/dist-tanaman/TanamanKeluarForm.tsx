...
export default function TanamanKeluarForm({
  onSuccess,
  initialData,
  id_tanaman,
}: Props) {
  ...
  const isEdit = !!initialData?.id_tanamankeluar;

  const [formData, setFormData] = useState<Partial<TanamanKeluarData>>({
    id_tanaman:
  ...
  const handleSubmit = async () => {
    ...
    for (const field of requiredFields) {
      const val = formData[field as keyof TanamanKeluarData];
      if (!val || val === "") {
        setAlertMessage({
          variant: "warning",
    ...
    try {
      ...
      const endpoint = "/api/tanamankeluar";
      const response = isEdit
        ? await api.put(`${endpoint}/${initialData?.id_tanamankeluar}`, payload)
  ...
  return (
    <ComponentCard title="Form Input Tanaman Keluar">
      {alertMessage && (
